/** Camera / gallery picker for Schaden photos. */
...
function assetToPhoto(asset?: Asset | null): PickedPhoto | null {
  ...
  const name = asset.fileName || `schaden-${Date.now()}.jpg`
...
export function pickJobPhoto(): Promise<PickedPhoto | null> {
  return new Promise(resolve => {
    Alert.alert('Schadenfoto', 'Quelle wählen', [
      {
        text: 'Kamera',
        onPress: () => {
          void launchCamera(
...
export function photoFormData(photo: PickedPhoto, kind: 'before' | 'after' | 'other' = 'before') {
